...
object U2bPlayerRestApiSync {
    ...
    fun getVideos(searchTerm: String): YTVideos {
        val logStr = "getVideos"
        LogUtil.d(TAG, "$logStr.searchTerm = $searchTerm")
        var result = YTVideos(videos = emptyList())
        try {
            val response: Response<YTVideos> = getApiInstance()
            ...
            if (code == HTTP_OK) {
                result = response.body() ?: YTVideos(videos = emptyList())
            }
        } catch (ex: Exception) {
